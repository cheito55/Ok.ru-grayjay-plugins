...
function extractSearchResults(html) {
    ...
    // OK.ru changes result-card markup frequently. Collect video hrefs first
    // instead of requiring one exact <a>...</a> structure.
    let hrefRe = /(?:href|data-href|data-url)\s*=\s*["']([^"']+)["']/gi;
    let m;

    while ((m = hrefRe.exec(html)) !== null && results.length < MAX_SEARCH) {
        let href = htmlDecode(m[1] || '').replace(/\\\//g, '/');
        let idm = href.match(/(?:https?:\/\/(?:www\.)?ok\.ru)?\/video\/(\d+)(?:[/?#]|$)/i);
        if (!idm) continue;
        ...
        let pos = m.index;
        let block = html.substring(Math.max(0, pos - 3000), Math.min(html.length, hrefRe.lastIndex + 6000));
        let title = '';

        let titlePatterns = [
            /data-video-title\s*=\s*["']([^"']{2,1000})["']/i,
            /data-title\s*=\s*["']([^"']{2,1000})["']/i,
            /aria-label\s*=\s*["']([^"']{2,1000})["']/i,
            /title\s*=\s*["']([^"']{2,1000})["']/i,
            /(?:video-title|movie-title|video_name|videoName|movieName)[^:=]{0,40}[:=]\s*["']([^"']{2,1000})["']/i,
            /["'](?:title|name|videoTitle|video_title)["']\s*:\s*["']([^"']{2,1000})["']/i
        ];
        for (let i = 0; i < titlePatterns.length; i++) {
            let tm = block.match(titlePatterns[i]);
            if (tm && cleanText(tm[1]).length >= 2) { title = cleanText(tm[1]); break; }
        }

        if (!title) {
            let tm2 = block.match(/<(?:h1|h2|h3|span|div)[^>]*class=["'][^"']*(?:title|name)[^"']*["'][^>]*>([\s\S]{2,1200}?)<\/(?:h1|h2|h3|span|div)>/i);
            if (tm2) title = cleanText(tm2[1]);
        }

        let poster = '';
        let posterPatterns = [
            /(?:data-poster|data-thumbnail|data-thumb|data-preview|poster|thumbnail|og:image)\s*=\s*["']([^"']+)["']/i,
            /["'](?:poster|thumbnail|thumbnailUrl|preview|image|imageUrl)["']\s*:\s*["']([^"']+)["']/i
        ];
        for (let i = 0; i < posterPatterns.length; i++) {
            let pm = block.match(posterPatterns[i]);
            if (pm) { poster = normalizeUrl(pm[1]); if (poster) break; }
        }
        if (!poster) {
            let im = block.match(/<img[^>]+(?:src|data-src|data-lazy-src|data-original)\s*=\s*["']([^"']+)["']/i);
            if (im) poster = normalizeUrl(im[1]);
        }

        let duration = 0;
        let dm = block.match(/(?:duration|movie-duration|video-duration|data-duration)[^>:=]{0,100}(?:[:=]\s*["']?|>)[^<"']{0,80}?([0-9]{1,2}:[0-9]{2}(?::[0-9]{2})?)/i);
        if (dm) duration = parseDurationText(dm[1]);

        if (!title) title = 'OK.ru video ' + id;
        seen[id] = true;
        results.push({ id: id, url: 'https://ok.ru/video/' + id, title: title, thumbnail: poster, duration: duration });
    }
    ...
    if (results.length === 0) {
        ...
        while ((m = re.exec(html)) !== null && results.length < MAX_SEARCH) {
            ...
            if (dm) duration = parseDurationText(dm[1]);
            seen[id] = true;
            results.push({ id: id, url: 'https://ok.ru/video/' + id, title: title || 'OK.ru video ' + id, thumbnail: poster, duration: duration });
        }
    }
    return results;
}
