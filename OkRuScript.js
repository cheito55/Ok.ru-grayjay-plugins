...
function getPoster(meta, baseUrl) {
    return normalizeUrl(firstValue(meta, [
        ...
        "image", "imageUrl", "preview", "previewUrl"
    ]), baseUrl);
}
...
function collectYouTubePlayerApiMedia(videoId) {
    ...
    for (let i = 0; i < endpoints.length; i++) {
        try {
            ...
            let r = http.POST(endpoints[i], bodies[i], headers);
            if (!r) continue;
            ...
            for (let j = 0; j < lists.length && out.mp4.length < 24; j++) {
                let f = lists[j] || {};
                if (!isHttpUrl(f.url)) continue;
...
/*
 * OK.ru suele entregar los bloques de resultados de búsqueda embebidos
 * dentro de JSON dentro del HTML (por ejemplo dentro de un valor de tipo
 * "innerVideoContent"). En ese caso las comillas del atributo aparecen
 * escapadas: data-movie-id=\"12345\" en vez de data-movie-id="12345".
 * Un regex que solo espera comillas literales nunca matchea eso y el
 * buscador devuelve 0 resultados sin ningún error visible.
 * Esta funcion normaliza ambas variantes para que el mismo regex
 * funcione sin importar si el fragmento venia crudo o escapado.
 */
function unescapeEmbeddedHtml(s) {
    return htmlDecode(safeStr(s))
        .replace(/\\\//g, "/")
        .replace(/\\u002f/gi, "/")
        .replace(/\\u003a/gi, ":")
...
/*
 * Estrategia principal: DOMParser (ya declarado en el config del plugin
 * y usado en la extracción de comentarios). Es mucho más robusta que un
 * regex porque no depende de en qué orden aparezcan los atributos dentro
 * de la tarjeta, ni de si el HTML viene con comillas escapadas.
 */
function extractSearchResultsDom(html) {
    let results = [];
    let seen = {};
    ...
    try {
        let doc = new DOMParser().parseFromString(html, "text/html");
        let nodes = doc.querySelectorAll(
            "[data-movie-id], .video-card a[href*='/video/'], .js-movie-card a[href*='/video/'], a[href*='/video/']"
        );

        for (let i = 0; i < nodes.length && results.length < MAX_SEARCH; i++) {
            let node = nodes[i];
            let id = "";

            try { id = node.getAttribute("data-movie-id") || ""; } catch (_) {}

            if (!id) {
                try {
                    let href = node.getAttribute("href") || "";
                    let hm = href.match(/\/video\/(\d+)/);
                    if (hm) id = hm[1];
                } catch (_) {}
            }
            ...
            if (!id) {
                try {
                    ...
                    let om = onclick.match(/openMovie\(\s*["'](\d+)["']/);
                    if (om) id = om[1];
                } catch (_) {}
            }
            ...
            // Subimos hasta el contenedor de la tarjeta para tener
            // título/poster/duración a mano, sin asumir en qué orden
            // aparecen los atributos dentro del bloque.
            let card = node;
            try {
                for (let up = 0; up < 5 && card && card.parentElement; up++) {
                    let cls = "";
                    try { cls = card.getAttribute && (card.getAttribute("class") || ""); } catch (_) {}
                    if (/video-card|ugrid_i/i.test(cls)) break;
            ...
            try {
                let img = card && card.querySelector ? card.querySelector("img[alt]") : null;
                if (img) title = cleanText(img.getAttribute("alt") || "");
            } catch (_) {}

            if (!title) {
                try {
                    let label = card && card.querySelector ? card.querySelector("[aria-label]") : null;
                    if (label) title = cleanText(label.getAttribute("aria-label") || "");
                } catch (_) {}
            }

            try {
                let img2 = card && card.querySelector
                    ? (card.querySelector("img[src*='videoPreview']") || card.querySelector("img[src]"))
                    : null;
                if (img2) poster = normalizeUrl(img2.getAttribute("src") || "");
            } catch (_) {}

            try {
                let durNode = card && card.querySelector
                    ? card.querySelector(".video-card_duration, [class*='duration']")
                    : null;
                if (durNode) duration = parseDurationText(cleanText(durNode.textContent || ""));
            } catch (_) {}
        ...
        addDebug("search DOM=" + e);
    }

    return results;
}

function extractResultBlock(id, block, seen) {
    ...
    // Título: alt="..." del thumbnail, data-title/title, o texto de un
    // contenedor con clase title/name.
    let tm = block.match(/alt\s*=\s*["']([^"']+)["']/i);
    if (tm) title = cleanText(tm[1]);

    if (!title) {
        tm = block.match(/(?:data-title|title|aria-label)\s*=\s*["']([^"']+)["']/i);
        if (tm) title = cleanText(tm[1]);
    }
    ...
    // Poster: src="..." de imagen, o atributos poster/thumbnail/data-poster.
    let pm = block.match(/\bsrc\s*=\s*["']([^"']+videoPreview[^"']*)["']/i);
    if (!pm) pm = block.match(/(?:poster|thumbnail|data-poster-src|data-poster|data-options)\s*=\s*["']([^"']+)["']/i);
    if (pm) poster = normalizeUrl(pm[1]);

    // Duración: texto visible tipo "09:07" o atributo numérico en ms.
    let dm = block.match(/video-card_duration[^>]*>\s*([0-9:]{1,12})\s*</i);
    if (!dm) dm = block.match(/(?:duration|movie-duration)[^>]*>([^<]{1,30})</i);
...
function extractSearchResults(html) {
    // Estrategia 1: DOMParser sobre el HTML tal cual, y si no encuentra
    // nada, sobre la versión "desescapada" (por si el fragmento venía
    // embebido dentro de JSON con comillas tipo \").
    let domResults = extractSearchResultsDom(html);
    if (!domResults.length) {
        let normalizedForDom = unescapeEmbeddedHtml(html);
        if (normalizedForDom !== safeStr(html)) {
            domResults = extractSearchResultsDom(normalizedForDom);
        }
    }
    if (domResults.length) return domResults;

    addDebug("search DOM found 0, falling back to regex");

    // Estrategia 2 (respaldo): regex tolerante a comillas escapadas,
    // por si DOMParser no está disponible en este runtime o el
    // documento no es HTML válido para parsear.
    let results = [];
    let seen = {};

    let raw = safeStr(html);
    let normalized = unescapeEmbeddedHtml(html);
    let variants = normalized !== raw ? [raw, normalized] : [raw];

    // Estrategia 1 (preferida): bloque delimitado por data-movie-id="ID".
    let reMovieId = /data-movie-id\s*=\s*["']?(\d+)["']?([\s\S]{0,5000}?)(?=data-movie-id|$)/gi;

    // Estrategia 2 (fallback): href="/video/ID" o el onclick de
    // OK.VideoPlayer.openMovie("ID", ...), usado en variantes de markup
    // donde data-movie-id no está presente.
    let reFallback = /(?:href\s*=\s*["']\/video\/(\d+)|openMovie\(\s*["'](\d+)["'])([\s\S]{0,3000}?)(?=(?:href\s*=\s*["']\/video\/\d+|openMovie\(\s*["']\d+["'])|$)/gi;

    for (let v = 0; v < variants.length && results.length < MAX_SEARCH; v++) {
        let body = variants[v];
        let m;

        reMovieId.lastIndex = 0;
        while ((m = reMovieId.exec(body)) !== null && results.length < MAX_SEARCH) {
            let r = extractResultBlock(m[1], m[2] || "", seen);
            if (r) results.push(r);
        }

        reFallback.lastIndex = 0;
        while ((m = reFallback.exec(body)) !== null && results.length < MAX_SEARCH) {
            let id = m[1] || m[2];
            let r = extractResultBlock(id, m[3] || "", seen);
            if (r) results.push(r);
        }
    }
...
function searchOk(query) {
    ...
    addDebug("search url=" + url + " len=" + (html ? html.length : 0));
    if (!html) throw new Error("OK.ru search returned no data\n" + debugText());

    let raw = extractSearchResults(html);
    addDebug("search matches=" + raw.length);
    if (!raw.length) {
        // No se lanza excepción: GrayJay debe poder mostrar "sin resultados"
        // en vez de un error. El detalle queda en debugText() para
        // diagnóstico si se habilita la opción "debug" del plugin.
        addDebug("search sample=" + safeStr(html).substring(0, 500));
    }
    ...
    return new VideoPager(out, false, null);
}
...
function searchSuggestions(query) {
    ...
    try {
        ...
        for (let i = 0; i < raw.length && out.length < 10; i++) {
            if (raw[i].title) out.push(raw[i].title);
        }
    } catch (e) {
