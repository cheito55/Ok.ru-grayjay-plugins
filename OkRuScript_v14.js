...
// v14: caches the expensive page/details work. This mirrors the important
// architectural idea used by the official YouTube source: do not repeat the
// same network request for every UI access to the same item.
const PAGE_CACHE = {};
const DETAILS_CACHE = {};
const PAGE_CACHE_MAX = 24;

function cachePut(cache, key, value, max) {
    try {
        cache[key] = value;
...
function loadOkPage(url) {
    ...
    if (PAGE_CACHE[canonical]) {
        addDebug("page-cache=hit");
        return PAGE_CACHE[canonical];
    }

    // v14: anonymous first. Public OK.ru videos normally do not require the
    // authenticated HTTP client, and an auth-backed page GET can be much
    // slower than the native YouTube player request path. Authentication is
    // only a fallback when the public page is unusable.
    let attempts = [
        { url: canonical, ua: UA_DESKTOP, auth: false },
        { url: canonical, ua: UA_DESKTOP, auth: true },
        { url: id ? "https://m.ok.ru/video/" + id : "", ua: UA_MOBILE, auth: false }
    ];

    for (let i = 0; i < attempts.length; i++) {
        if (!attempts[i].url) continue;
        try {
            let body = attempts[i].auth
                ? httpGetAuth(attempts[i].url, { "User-Agent": attempts[i].ua })
                : httpGet(attempts[i].url, { "User-Agent": attempts[i].ua });
            if (body && body.length > 250) {
                addDebug("page=" + i + ",len=" + body.length + ",auth=" + attempts[i].auth);
                cachePut(PAGE_CACHE, canonical, body, PAGE_CACHE_MAX);
                return body;
            }
        } catch (_) {}
    }
...
function getVideoDetailsSync(url) {
    resetDebug();
    let id = extractVideoId(url);
    if (!id) throw new Error("Invalid OK.ru video URL");

    let canonical = "https://ok.ru/video/" + id;
    if (DETAILS_CACHE[canonical]) {
        addDebug("details-cache=hit");
        return DETAILS_CACHE[canonical];
    }
    ...
    // One and only one metadata endpoint fallback. No sleep/retry loop.
    ...
    // A YouTube embed is deliberately not resolved here. Returning a fake
    // empty descriptor makes GrayJay wait for a source that this plugin cannot
    // legitimately provide. Keep the external URL in the detail object only
    // when the runtime accepts it as the content URL.
    if (external && merged.hls.length === 0 && merged.mp4.length === 0) {
        ...
        let obj = {
            ...
            video: makeEmptyDescriptor() || {},
            dash: null,
            hls: null,
            live: []
        };
        let result;
        try { result = new PlatformVideoDetails(obj); }
        catch (_) { result = new PlatformVideoDetails({
            ...
            video: obj.video, live: []
        }); }
        cachePut(DETAILS_CACHE, canonical, result, PAGE_CACHE_MAX);
        return result;
    }
    ...
    if (merged.hls.length || merged.mp4.length) {
        ...
        let result = buildDetails(meta, canonical, id);
        cachePut(DETAILS_CACHE, canonical, result, PAGE_CACHE_MAX);
        return result;
    }
...
// GrayJay's official YouTube source uses Promise-returning content details in
// its session-client path. Yielding before the synchronous OK.ru network work
// prevents the search screen from being held by the detail resolver itself.
function getVideoDetails(url) {
    return new Promise(function(resolve, reject) {
        setTimeout(function() {
            try { resolve(getVideoDetailsSync(url)); }
            catch (e) { reject(e); }
        }, 0);
    });
}
