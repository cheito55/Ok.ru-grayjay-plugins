...
function httpGetAuthenticated(url) {
    try {
        // GrayJay's HTTP API accepts the login/session flag as the third
        // argument. This makes the runtime attach its managed OK.ru cookies
        // instead of inventing or hard-coding cookie values.
        let useLogin = false;
        try { useLogin = !!bridge.isLoggedIn(); } catch (_) {}
        let r = http.GET(url, {"Accept-Language":"en-US"}, useLogin);
        if (!r) return "";
