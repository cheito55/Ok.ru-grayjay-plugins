...
function containsYouTubeEmbed(value) {
    let s = safeStr(value)
        .replace(/\\u002F/gi, "/")
        .replace(/\\\//g, "/")
        .replace(/&amp;/gi, "&");
    return /(?:youtube(?:-nocookie)?\.com\/(?:embed|watch|shorts|live|v)\/|youtu\.be\/[A-Za-z0-9_-]{6,20}|(?:youtubeId|youtubeVideoId|externalVideoId)\s*[=:])/i.test(s);
}
...
function extractSearchResults(html) {
    ...
    // Current OK.ru markup: normal /video/<id> links.
    let anchorRe = /<a\b([^>]*?href\s*=\s*["'](?:https?:\/\/[^"']+)?(?:\/)?video\/(\d+)(?:[?#][^"']*)?["'][^>]*)>([\s\S]*?)<\/a>/gi;
    let m;

    while ((m = anchorRe.exec(html)) !== null && results.length < 30) {
        let id = m[2];
        if (!id || seen[id]) continue;
        let blockStart = Math.max(0, m.index - 2500);
        let blockEnd = Math.min(html.length, anchorRe.lastIndex + 2500);
        let block = html.substring(blockStart, blockEnd);
        if (containsYouTubeEmbed(block)) continue;
        ...
        if (dm) {
            let parts = dm[1].split(":");
            if (parts.length === 2) duration = (parseInt(parts[0],10)||0)*60+(parseInt(parts[1],10)||0);
            else if (parts.length === 3) duration = (parseInt(parts[0],10)||0)*3600+(parseInt(parts[1],10)||0)*60+(parseInt(parts[2],10)||0);
        }
    ...
    if (results.length === 0) {
        ...
        while ((m = re.exec(html)) !== null && results.length < 30) {
            ...
            let pm = block.match(/(?:poster|thumbnail|data-poster)\s*=\s*["']([^"']+)["']/i);
            if (pm) poster = normalizeUrl(pm[1]);
            let dm = block.match(/(?:duration|movie-duration)[^>]*>([^<]{1,30})</i);
            if (dm) {
                let parts = cleanText(dm[1]).split(":");
                if (parts.length === 2) duration = (parseInt(parts[0],10)||0)*60+(parseInt(parts[1],10)||0);
                else if (parts.length === 3) duration = (parseInt(parts[0],10)||0)*3600+(parseInt(parts[1],10)||0)*60+(parseInt(parts[2],10)||0);
            }
            seen[id] = true;
...
function searchOk(query) {
    ...
    // First request uses GrayJay's managed login cookies when available.
    // Fallback remains the original v5 unauthenticated request.
...
function doDetails(url) {
    ...
    // Exclude actual YouTube embeds, but allow ordinary OK.ru pages that merely
    // mention YouTube in text.
    if (containsYouTubeEmbed(html)) {
        throw new Error("YouTube embed excluded");
    }
