 ...
 *  - Los videos que sean embeds de YouTube se excluyen de los resultados.
 *  - Cast optimizado con URLs directas HLS primero y MP4 como fallback.
 *
...
/* YouTube direct resolvers intentionally removed: embeds are excluded. */
...
function blockHasYouTubeEmbed(block) {
    let s = safeStr(block);
    // Solo marca referencias que parecen un embed/video externo de YouTube.
    // No se filtra una pagina simplemente porque mencione "youtube" en texto.
...
function extractSearchResults(html) {
    ...
    while ((m = anchorRe.exec(html)) !== null && results.length < MAX_SEARCH) {
        ...
        let blockStart = Math.max(0, m.index - 3000);
        let blockEnd = Math.min(html.length, anchorRe.lastIndex + 3000);
        let block = html.substring(blockStart, blockEnd);

        // No mostrar en los resultados los OK.ru que son solo un embed de YouTube.
        if (blockHasYouTubeEmbed(block)) continue;
...
function getVideoDetails(url) {
    ...
    for (let attempt = 0; attempt < delays.length; attempt++) {
        ...
        if (external) {
            addDebug("YouTube embed excluded=" + external.id);
            // Este plugin solo reproduce fuentes directas de OK.ru.
            // Los embeds de YouTube no se muestran ni se resuelven aqui.
            continue;
        }
    ...
    // YouTube embeds are intentionally excluded from this source.
