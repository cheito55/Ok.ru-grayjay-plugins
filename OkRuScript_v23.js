...
function httpGetAuthenticated(url) {
    try {
        let useLogin = false;
        try {
            useLogin = !!(typeof bridge !== "undefined" && bridge &&
                typeof bridge.isLoggedIn === "function" && bridge.isLoggedIn());
        } catch (_) {}
        let r = http.GET(url, null, useLogin);
        if (!r) return "";
    ...
    for (let i = 0; i < raw.length; i++) {
        ...
        try {
            videos.push(new PlatformVideo({
                id: new PlatformID(PLATFORM_NAME, r.id, "62af0e2f-bfd9-489f-afe1-f66583d2f7d0"),
            ...
            addDebug("search video: " + e);
        }
    }
    return new OkSearchVideoPager(videos, false);
}
...
/* Official-YouTube-style pager wrapper.  The current GrayJay source uses
 * subclasses of VideoPager and calls super(videos, hasMore). */
class OkSearchVideoPager extends VideoPager {
    constructor(videos, hasMore) {
...
source.getHome = function () {
    return new OkSearchVideoPager([], false);
};
