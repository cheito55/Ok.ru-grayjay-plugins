...
function searchOk(query) {
    ...
    for (let i = 0; i < raw.length; i++) {
        ...
        try {
            videos.push(new PlatformVideo({
                ...
                thumbnails: r.thumbnail ? new Thumbnails([new Thumbnail(r.thumbnail, 0)]) : new Thumbnails([]),
                author: null,
                uploadDate: 0,
                url: r.url,
    ...
    // Current GrayJay expects a real Pager for source.search.
    // Keep the original v5 HTML/search extraction, but adapt only the
    // outer return type to the current Pager API.
    try {
...
source.getHome = function () {
    try { return new VideoPager([], false); }
    catch (_) { return new VideoPager([], false, null); }
};
